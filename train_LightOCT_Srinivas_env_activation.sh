...
declare -a dataset_type=Srinivas
declare -a dataset_split_strategy=per_image
declare -a number_test_images_per_class=250
declare -a ids=none
declare -a n_folds=5
declare -a n_rkf=10
...
python3 -u configure_training.py -wd $working_folder -df $dataset_folder -dt $dataset_type -dss $dataset_split_strategy -ntspc $number_test_images_per_class -mc $model_configuration -mn $save_model_name -b $batchSize -f $n_folds -nkf $n_rkf -l $loss -lr $lr -ids $ids -v 2 -db False -rle $random_label_experiment |& tee $log_folder/$save_model_name.log
