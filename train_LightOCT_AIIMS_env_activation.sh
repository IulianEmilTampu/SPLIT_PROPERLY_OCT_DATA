...
python3 -u configure_training.py -wd $working_folder -df $dataset_folder -dt $dataset_type -dss $dataset_split_strategy -mc $model_configuration -mn $save_model_name -b $batchSize -f $n_folds -nkf $n_rkf -l $loss -lr $lr -ids $ids -v 2 -db False -rle $random_label_experiment -ctd False |& tee $log_folder/$save_model_name.log
